...
class AuthControllerCore extends FrontController
{
	...
	protected function processSubmitLogin()
	{
		...
		if (empty($email))
			$this->errors['email'] = Tools::displayError('An email address required.');
		elseif (!Validate::isEmail($email))
			$this->errors['email'] = Tools::displayError('Invalid email address.');
		elseif (empty($passwd))
			$this->errors['passwd'] = Tools::displayError('Password is required.');
		elseif (!Validate::isPasswd($passwd))
			$this->errors['passwd'] = Tools::displayError('Invalid password.');
		else
		{
	...
	protected function processSubmitAccount()
	{
		...
		if ($error_phone){
			$this->errors['phone'] = Tools::displayError('You must register at least one phone number.');
		}
		...
		else // if registration type is in one step, we save the address
		{
			...
			foreach($addresses_types as $addresses_type)
			{
				...
				if (!($country = new Country($$addresses_type->id_country)) || !Validate::isLoadedObject($country))
					$this->errors['country_'.$addresses_type] = Tools::displayError('Country cannot be loaded with address->id_country');
				$postcode = Tools::getValue('postcode');		
				/* Check zip code format */
				if ($country->zip_code_format && !$country->checkZipCode($postcode))
					$this->errors['postcode_'.$addresses_type] = sprintf(Tools::displayError('The Zip/Postal code you\'ve entered is invalid. It must follow this format: %s'), str_replace('C', $country->iso_code, str_replace('N', '0', str_replace('L', 'A', $country->zip_code_format))));
				elseif(empty($postcode) && $country->need_zip_code)
					$this->errors['postcode_'.$addresses_type] = Tools::displayError('A Zip / Postal code is required.');
				elseif ($postcode && !Validate::isPostCode($postcode))
					$this->errors['postcode_'.$addresses_type] = Tools::displayError('The Zip / Postal code is invalid.');
				...
				if (Tools::isSubmit('submitAccount') || Tools::isSubmit('submitGuestAccount'))
					if (!($country = new Country($$addresses_type->id_country, Configuration::get('PS_LANG_DEFAULT'))) || !Validate::isLoadedObject($country))
						$this->errors['country_'.$addresses_type] = Tools::displayError('Country is invalid');
				...
				if ((Tools::isSubmit('submitAccount')|| Tools::isSubmit('submitGuestAccount')) && $contains_state && !$id_state)
					$this->errors['state_'.$addresses_type] = Tools::displayError('This country requires you to choose a State.');
			}
		}
			...
			if (!count($this->errors))
			{
				...
				else
				{
					...
					if (!count($this->errors))
					{
						...
						// redirection: if cart is not empty : redirection to the cart
						if (count($this->context->cart->getProducts(true)) > 0){
							//die('index.php?controller=order&multi-shipping='.(int)Tools::getValue('multi-shipping'));
							Tools::redirect('index.php?controller=order&multi-shipping='.(int)Tools::getValue('multi-shipping').'&step=2');
						}
						// else : redirection to the account
						else{
							//die('auth redirect');
