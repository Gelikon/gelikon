...
$(document).ready(function () {
    ...
    function initialize(callbacks) {
        callbacks.each(function () {
            ...
            poster.hide().eq(0).show();
            button.on("click", function () {
                var tabIndex = $(this).index();
                button.removeClass("sel");
                $(this).addClass("sel");
                poster.hide().eq(tabIndex).show();

                if (tabIndex == 0) {
                    button.eq(0).html('на русском');
                    button.eq(1).html('на немецком')
                } else {
                    button.eq(0).html('auf russisch');
                    button.eq(1).html('deutsch')
                }
        ...
        $(".gk-blk-open-radio").each(function () {
            $(".jq-radio").on("click", function () {
                ...
                var relatedTarget = $(this),
                    delSum = parseFloat(relatedTarget.closest(".wrap").find(".price").html()),
                    totalSum = parseFloat(($('#value').val()));
                ...
                $(".total-price span").html(totalSum+delSum + ' ' + 'EUR')
            });
        });
    })();
