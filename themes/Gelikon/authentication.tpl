...
{capture name=path}
	...
	{/if}
{/capture}
{if isset($ajax_isset) && $ajax_isset}
<script>
    $(function(){
       $("select, input").styler(); 
    });
...
<h1 class="gk-align-C">{if !isset($email_create)}{l s='Authentication'}{else}{l s='Create an account'}{/if}</h1>
				...
				<form {*method="post" action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}"*} onsubmit="myFunction(); return false;"> {* Gelikon hack here *}
					...
					<div class="hide-panel">
						<div class="gk-align-R">
							<button type="submit" id="button_not_register_instant_chackout" class="btn btn-yellow">{l s='продолжить'}</button>
						</div>
						<script type="text/javascript">
						{*  Gelikon hack here *}
						{*
$("#button_not_register_instant_chackout").click(function() {

    $('html, body').animate({
        scrollTop: $("#opc_account_form").offset().top
    }, 2000);
...
*}
function myFunction(){
	$("#new_account_form").css( "display", "block");
	 $('html, body').animate({
        scrollTop: $("#new_account_form").offset().top
			...
			<div class="wrap {if $auth_type === '2'}sel{/if}">
				<form id="create-account_form" method="post" action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}">
			...
			<div class="wrap {if $auth_type === '3'}sel{/if}" >
				<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html':'UTF-8'}" method="post" id="login_form">
	...
	{if isset($inOrderProcess) && $inOrderProcess && $PS_GUEST_CHECKOUT_ENABLED}
		<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std clearfix" style="display:none;"> {*Gelikon hack here*}
                        {if isset($ajax_isset) && $ajax_isset}
                        <div class="alert alert-danger" id="create_account_error_ajax" style="display:none"></div>
                        {/if}
			<div class="box">
			...
			<p class="cart_navigation required submit clearfix">
				<span><sup>*</sup>{l s='Required field'}1</span>
				<input type="hidden" name="display_guest_checkout" value="1" />
