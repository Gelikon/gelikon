...
<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm">
	<div class="select selector1 sort_select">
		...
		<select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort form-control">
			<option value="{$orderbydefault|escape:'html':'UTF-8'}:{$orderwaydefault|escape:'html':'UTF-8'}">{l s='По новизне вперед'}</option>
                        {if $orderby eq 'bestsale' AND $orderway eq 'asc'}
                        {else}
                        <option value="bestsale:asc">{l s='По бестселлерам вперед'}</option>
                        {/if}
                        {if $orderby eq 'bestsale' AND $orderway eq 'desc'}
                        {else}
			<option value="bestsale:desc">{l s='По бестселлерам назад'}</option>
                        {/if}
                        {if $orderby=='date_add' AND $orderway=='asc'}
                        {else}
                        <option value="date_add:asc">{l s='По новизне вперед'}</option>
                        {/if}
                        {if $orderby=='date_add' AND $orderway=='desc'}
                        {else}
			<option value="date_add:desc">{l s='По новизне назад'}</option>
                        {/if}
		</select>
	</div>
</form>
