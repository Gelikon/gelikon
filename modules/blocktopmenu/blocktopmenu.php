...
class Blocktopmenu extends Module
{
	...
	private function makeMenu()
	{
		...
		foreach ($menu_items as $item)
		{
			...
			{
				case 'CAT':
					$temp = Category::getNestedCategories($id, $id_lang, true, $this->user_groups);
					//print_r($temp);die;
					$this->_menu .= $this->generateCategoriesMenu($temp);
					$this->menu_item['cat'][$id] = $temp[$id];

					break;

				case 'PRD':
					$this->menu_item['prd'][] = $id;
	...
	private function generateCategoriesMenu($categories)
	{
		...
		foreach ($categories as $key => $category)
		{
			...
			$html .= '<li class="'.(($this->page_name == 'category'
				&& (int)Tools::getValue('id_category') == (int)$category['id_category']) ? 'sfHoverForce ' : ' ').$class.'" >';
			$html .= '<a href="'.$link.'" title="'.$category['name'].'">'.$category['name'].'</a>';

			if (isset($category['children']) && !empty($category['children'])){

				/* denamit */
				$column = ceil(count($category['children'])/2);
				...
				/***********/
				$html .= '<ul>';
				$html .= $this->generateCategoriesSubMenu($category['children']);
	...
	private function generateCategoriesSubMenu($categories){
		$html = '';
	...
	public function hookDisplayTop($param)
	{
		...
		if (!$this->isCached('blocktopmenu.tpl', $this->getCacheId()))
		{
			...
			$this->smarty->assign('category', new Category());
		}
