...
*/
	...
	{
		$key = $this->getCacheId('blocksearch-footer'.((!isset($params['hook_mobile']) || !$params['hook_mobile']) ? '' : '-hook_mobile'));
		if (Tools::getValue('search_query') || !$this->isCached('blocksearch-footer.tpl', $key))
		{
			$this->calculHookCommon($params);
			$this->smarty->assign(array(
				'blocksearch_type' => 'footer',
				'search_query' => (string)Tools::getValue('search_query')
				)
			);
		}
		Media::addJsDef(array('blocksearch_type' => 'footer'));
		return $this->display(__FILE__, 'blocksearch-footer.tpl', Tools::getValue('search_query') ? null : $key);
	}
