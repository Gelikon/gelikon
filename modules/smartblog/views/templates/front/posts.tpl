...
<div class="blk gk-elem-last gk-blk-news gk-content-margin">
    <div class="item-wrap">
        {foreach from=$allNews item=item}
            {assign var="options" value=null}
            {$options.id_post = $item.id_post} 
            {$options.slug = $item.link_rewrite}
            <div class="item">
                 <div class="date">{$item.created|date_format:"%Y.%m.%d"}</div>
                  <a class="title" href="{smartblog::GetSmartBlogLink('smartblog_post',$options)}">{$item.meta_title}</a>
                  <div class="description">
                        <div class="text">
                            {$item.short_description}
                        </div>
                        <div class="link">
                            <a class="arrow-right" href="{smartblog::GetSmartBlogLink('smartblog_post',$options)}"></a>
                        </div>
                  </div>
            </div>
        {/foreach}
    </div>
    <a class="all-news" href="{smartblog::GetSmartBlogLink('smartblog')}">{l s="показать еще" mod="smartblog"}</a>
</div>
{/if}
