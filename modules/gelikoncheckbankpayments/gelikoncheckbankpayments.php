...
добавить загрузку файла - разделить отчет по записанным оплатам, по неоплаченным заказам, форму загрузки csv, отчет по загруженному csv
...
//предоставляемые им методы, а именно l(), который отвечает за локализацию строки
	...
	private $id_order_state_payed = 2;//статус "Платеж принят";
		    ...
		    while (($data = fgetcsv($handle, 1000, $delimiter,$enclosure,$escape)) !== FALSE) {
		        if ($row>9)
		        {
			        ...
			        if (count($data_filtered)>0) //не добавляем пустые строки
			        {
						...
						{	
							//08.05.2014
							$pattern = '/([0-9]{1,2}\.){2}[0-9]{4}/';
							$matches = '';
							preg_match($pattern, $data_filtered[1], $matches);
							$date = empty($matches[0])? '' : $matches[0];
							$date = explode('.', $date);
							$timestamp = mktime(0, 0, 0, $date[1], $date[0], $date[2]); //month day year - уточнить у Дениса. Сейчас 08.05.2014 - это 8 мая 2014
							$mysqltime = date ("Y-m-d H:i:s", $timestamp);
							echo $mysqltime;
							$this->payments[$payment_row]['date'] = $timestamp;//$data_filtered[1]; //TODO преобразовать в php дату с ч м с
	...
	private function checkPaymentsNeeded()
	{
		...
		foreach ($this->payments as $num_row => $payment){
			...
			if (($this->payments[$num_row]['payment_needed'] == false) AND (!empty($payment['reference'])))
			{
				$this->_html .= $this->displayError($this->l('Заказ '.$this->payments[$num_row]['reference'].' со счётом '.$this->payments[$num_row]['invoice_num'].' уже оплачен.'));
			}
		}
	}
				...
				if (!empty($payment['reference']))
				{
					if ($payment['payment_needed'] == true)
					{
						...
						if (($id_order_payment==true) AND ($id_order == true ))
						{
							...
							echo "Заказ ".$payment['reference']." сохранена оплата заказа";
							echo "Добавляем оплату счёта ".$payment['invoice_num']."<br/>";
							$this->addOrderInvoicepayment($payment,$id_order_payment, $id_order);
							$this->setOrderStatePaid($payment,$id_order);
						}
					}
					else 
						$this->_html .= $this->displayError($this->l('Попытка оплатить оплаченный заказ '.$payment['reference'].' со счётом '.$payment['invoice_num']));	
				}
				else
				{
					echo "Невозможно оплатить ненайденный заказа со счётом ".$payment['invoice_num'];
				}
			}
	...
	private function updateOrderRealPayed($payment)
	{
		...
		if ($row = $db->getRow($sql))
		{
			...
			$sql = "UPDATE  `"._DB_NAME_."`.`"._DB_PREFIX_."orders` SET  `total_paid_real` =  '".$add_sum."', `date_upd` = FROM_UNIXTIME(".$payment['date'].") WHERE  `"._DB_PREFIX_."orders`.`reference` ='". pSQL($payment['reference'])."'";
			if ($db->execute($sql))
			{
				//писать в лог
	...
	private function setOrderStatePaid($payment,$id_order)
	{
		//проверяем, что сумма оплат не меньше чем сумма заказа
		$db = Db::getInstance();
		$sql = "SELECT `total_paid`, `total_paid_real`
			FROM  `"._DB_PREFIX_."orders` 
			WHERE `id_order` = '".$id_order."'";
		if ($row = $db->getRow($sql))
		{
			if ($row['total_paid_real']>=$row['total_paid'])
			{
				//добавляем в order_history
				...
				$sql = "
				INSERT INTO  `u9294_gelikon`.`ps_order_history` (
				`id_order_history` ,
				`id_employee` ,
				`id_order` ,
				`id_order_state` ,
				...
				VALUES (
				NULL,  '".$id_employee."',  '".$id_order."',  '".$this->id_order_state_payed."',  NOW()
				)";
				echo "Добавляем строку в order-history ".$sql;
				
				if ($db->execute($sql))
					echo "В историю статусов добавлена строка ".$db->Insert_ID();
				else
					echo "При добавлении записи в историю статусов заказа ".$payment['reference']." возникла ошибка ".$db->getMsgError();
				
				//меняем статус в orders
				$sql = "UPDATE  `"._DB_NAME_."`.`"._DB_PREFIX_."orders` SET  `current_state` =  '".$this->id_order_state_payed."' WHERE  `"._DB_PREFIX_."orders`.`id_order` =".$id_order;
				if ($db->execute($sql))
					echo "Обновлен статус заказа ".$id_order." номер ".$payment['reference']."<br/>";
				else
					$this->_html .= $this->displayError($this->l('Ошибка обновления статуса заказа '.$payment['reference'].', счёт : '.$this->payments[$num_row]['reference'].'. '.$dbErrorMsg));
				return false;
			}
			else
				echo "Сумма оплаты меньше стоимости заказа ".$payment['reference'];
		}
		else
			echo "При обновлении статуса заказа ".$payment['reference']." не смог получить сумму заказа и сумму ранее оплаченного";
	}
