...
class Import extends Module {
    ...
    private function upsert($line) {
        ...
        $product->depends_on_stock = 1; //0 - доступное количество на основе ASM. 1 - указывается вручную
        $product->out_of_stock = 1; //2 - как в Preferences product. 1 - allow
        ...
        $product->save();
        if ($product->name['1']=="Практикум по психологии личности") {
           //var_dump($product);  
        }
       
//StockAvailable::setProductDependsOnStock($this->id, true, null, 0);
...
/*
        ...
        StockAvailable::setProductOutOfStock($product->id, 1, null);
