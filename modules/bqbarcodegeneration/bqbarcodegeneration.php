...
class Bqbarcodegeneration extends Module
{
	...
	public function hookAddProduct($params)
	{
		...
		if (Tools::getValue('bean', $this->bean) == 1)
		{
			...
			Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'product SET ean13 = "'.$ean13.'" 
			WHERE (ean13 IS NULL OR ean13 = "" OR ean13 = 0) AND id_product = "'.$id.'"');
		}
		$products_attr_to_update_code = Db::getInstance()->ExecuteS('SELECT `id_product`,`id_product_attribute`
		FROM '._DB_PREFIX_.'product_attribute WHERE (ean13 IS NULL OR ean13 = "" OR ean13 = "0") AND id_product = "'.$id.'"');
		...
		if (Tools::getValue('bupc', $this->bupc) == 1)
		{
			...
			Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'product SET upc = '.$upc.' WHERE (upc IS NULL OR upc = "" OR upc = 0) AND id_product = "'.$id.'"');
		}
		$products_attr_to_update_code = Db::getInstance()->ExecuteS('SELECT `id_product`,`id_product_attribute`
		FROM '._DB_PREFIX_.'product_attribute WHERE (upc IS NULL OR upc = "" OR ean13 = "0") AND id_product = "'.$id.'"');
		if ($products_attr_to_update_code)
			foreach ($products_attr_to_update_code as $product_attr)
			{
				$upc = '';
	...
	public function generateEAN($number, $gs1, $type = 'ean13')
	{
		if ($type == 'ean13')
			$code = $gs1.str_pad($number, 9, '0', STR_PAD_LEFT);
		if ($type == 'upc')
			$code = $gs1.str_pad($number, 8, '0', STR_PAD_LEFT);
		$weightflag = true;
		$sum = 0;
