...
*
* @short description    PHP class for generating PDF documents without requiring external extensions.
*
* @author    Nicola Asuni - info@tecnick.com
*
* @copyright info@tecnick.com
*
* @version    1.0.002
*
...
class TcpdfStatic
{
	 ...
	 * @public static
	 */
	public static function set_mqr()
	{
	...
	public static function getPageSizeFromFormat($format)
	{
		...
		switch (Tools::strtoupper($format)) 
		{
	...
	public static function getBorderMode($brd, $position = 'start', $opencell = true)
	{
					...
					if (strpos($border, 'T') !== false)
		{
						...
						if (Tools::strlen($newkey) > 0) $brd[$newkey] = $style;
						unset($brd[$border]);
					}
					break;
				}
	...
	{
		return Tools::substr_replace(date('YmdHisO', (int)($time)), '\'', (0 - 2), 0).'\'';
	}
	/**
	...
	{
		$v = unpack('Ni', Tools::substr($str, $offset, 4));
		return $v['i'];
	}
	/**
	...
	{
		$v = unpack('ni', Tools::substr($str, $offset, 2));
		return $v['i'];
	}
	/**
	...
	{
		$v = unpack('si', Tools::substr($str, $offset, 2));
		return $v['i'];
	}
	/**
	...
	{
		$v = unpack('Ci', Tools::substr($str, $offset, 1));
		return $v['i'];
	}
	/**
	...
	{
		$seed .= microtime();
		if (function_exists('openssl_random_pseudo_bytes') && (Tools::strtoupper(Tools::substr(PHP_OS, 0, 3)) !== 'WIN'))
	...
	public static function _AES($key, $text)
	{
		...
		$padding = 16 - (Tools::strlen($text) % 16);
	...
	public static function _RC4($key, $text, &$last_enc_key, &$last_enc_key_c)
	{
		...
		if ($last_enc_key != $key)
		{
			$k   = str_repeat($key, ((256 / Tools::strlen($key)) + 1));
			$rc4 = range(0, 255);
			$j   = 0;
			for ($i = 0; $i < 256; ++$i)
		{
...
else	$rc4 = $last_enc_key_c;
		
		$len = Tools::strlen($text);
		$a   = 0;
		$b   = 0;
		$out = '';
		for ($i = 0; $i < $len; ++$i)
	...
	public static function getUserPermissionCode($permissions, $mode = 0)
	{
				...
				if (($mode > 0) || ($options[$permission] <= 32))
		{
			...
			else $protection -= $options[$permission];
		}
	}
}
		return $protection;
	...
	public static function getEncPermissionsString($protection)
	{
		...
		$str     = chr(bindec(Tools::substr($binprot, 24, 8)));
		$str .= chr(bindec(Tools::substr($binprot, 16, 8)));
		$str .= chr(bindec(Tools::substr($binprot, 8, 8)));
		$str .= chr(bindec(Tools::substr($binprot, 0, 8)));
		return $str;
	}
	/**
	...
	public static function encodeNameObject($name)
	{
		...
		for ($i = 0; $i < $length; ++$i)
		{
			...
			$escname .= $chr;
			else $escname .= sprintf('#%02X', ord($chr));
		}
		return $escname;
	}
	/**
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl = false)
	{
		...
		if (isset($prop['fillColor']))
		{
			...
			else $opt['mk']['bg'] = TCPDF_COLORS::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
		...
		if (isset($prop['strokeColor']))
		{
			...
			else	$opt['mk']['bc'] = TCPDF_COLORS::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
		...
		{	if ($prop['display'] == 'display.visible') $f += 0;
		...
		{
				$opt['opt'] = array();
				foreach (array_flip($prop['value']) as $key)
		{
	...
	public static function extractCSSproperties($cssdata)
	{
		...
		if (Tools::substr($cssdata, -1) == '}')
		
			// remove last parethesis
			$cssdata = Tools::substr($cssdata, 0, -1);
	...
	public static function fixHTMLCode($html, $default_css = '', $tagvs = '', $tidy_options = '', &$tagvspaces)
	{
		...
		if (isset($matches[1]))
			...
			$css = Tools::strtolower($matches[1]);
	...
	public static function isValidCSSSelectorForTag($dom, $key, $selector)
	{
		...
		if (isset($dom[$key]['attribute']['class']) && !empty($dom[$key]['attribute']['class']))
			...
			$class = explode(' ', Tools::strtolower($dom[$key]['attribute']['class']));
		
		$id = '';
		if (isset($dom[$key]['attribute']['id']) && !empty($dom[$key]['attribute']['id']))
		
			$id = Tools::strtolower($dom[$key]['attribute']['id']);
					...
					switch ($attrib{0})
		{
						case '.':
		{ // class
							if (in_array(Tools::substr($attrib, 1), $class))
		...
		{
								$att = Tools::strtolower($attrmatch[1]);
								$val = $attrmatch[3];
										...
										case '~=':
		{
										...
										case '^=':
		{
											if ($val == Tools::substr($dom[$key]['attribute'][$att], 0, Tools::strlen($val)))
										...
										case '$=':
		{
											if ($val == Tools::substr($dom[$key]['attribute'][$att], -Tools::strlen($val)))
	...
	{	// pseudo-class or pseudo-element
					...
					if ($valid && ($offset > 0))
		{
					...
					$selector = Tools::substr($selector, 0, $offset);
					switch ($operator)
		{
						case ' ':
	...
	public static function getCSSdataArray($dom, $key, $css)
	{
		...
		foreach ($css as $selector => $style)
		{
			$pos         = strpos($selector, ' ');
			// get specificity
			$specificity = Tools::substr($selector, 0, $pos);
			// remove specificity
			$selector    = Tools::substr($selector, $pos);
	...
	/**
	 * Find position of last occurrence of a Tools::substring in a string
	 * @param $haystack (string) The string to search in.
	 * @param $needle (string) Tools::substring to search.
	...
	{
		$length = Tools::strlen($haystack);
		$offset = ($offset > 0) ? ($length - $offset) : abs($offset);
		$pos    = strpos(strrev($haystack), strrev($needle), $offset);
		return ($pos === false) ? false : ($length - $pos - Tools::strlen($needle));
	}
	/**
	...
	public static function getHyphenPatternsFromTEX($file)
	{
		...
		$data           = trim(Tools::substr($matches[0], 10, -1));
	 ...
	 * @param $limit (int)If specified, then only Tools::substrings up to limit are returned with the rest of the string being placed in the last Tools::substring. A limit of -1, 0 or null means "no limit" and, as is standard across PHP, you can use null to skip to the flags parameter.
	 * @param $flags (int)The flags as specified on the preg_split PHP function.
	 * @return Returns an array containing Tools::substrings of subject split along boundaries matched by pattern.modifier
	 * @author Nicola Asuni
	 * @since 6.0.023
	 * @public static
	 */
	public static function pregSplit($pattern, $modifiers, $subject, $limit = null, $flags = null)
	{
		...
		{
			$ret     = array_merge($ret, preg_split($pattern.$modifiers, Tools::substr($subject, 0, $nl), $limit, $flags));
			$ret[]   = "\n";
			$subject = Tools::substr($subject, ($nl + 1));
		}
		if (Tools::strlen($subject) > 0)
	...
	public static function fileGetContents($file)
	{
		...
		if ((Tools::strlen($file) > 1) && ($file[0] == '/') && ($file[1] != '/') && !empty($_SERVER['DOCUMENT_ROOT']) && ($_SERVER['DOCUMENT_ROOT'] != '/'))
		{
			$findroot = strpos($file, $_SERVER['DOCUMENT_ROOT']);
			if (($findroot === false) || ($findroot > 1))
		{
				if (Tools::substr($_SERVER['DOCUMENT_ROOT'], -1) == '/')
		
					$tmp = Tools::substr($_SERVER['DOCUMENT_ROOT'], 0, -1).$file;
				
				else $tmp = $_SERVER['DOCUMENT_ROOT'].$file;
		...
		if (!isset($urldata['query']) || (Tools::strlen($urldata['query']) <= 0))
		{
		...
		foreach ($alt as $f)
		{
			$ret = @Tools::file_get_contents($f);
			if (($ret === false) && !ini_get('allow_url_fopen') && function_exists('curl_init') && preg_match('%^(https?|ftp)://%', $f))
		{
