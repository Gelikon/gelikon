...
<script type="text/javascript">
	...
	$(document).ready(function() {
		/*для Gelikon */
		$('#setup-retail-customer').focus();
		$('#setup-retail-customer').on('click',function(e){
			$('#customer').val('Розничный Покупатель');
			searchCustomers();
			setupCustomer(5);

		});

		$('#customers').bind("DOMSubtreeModified",function(){
  			console.log('changed');
  			console.log('меняем'); 
  			var n = $('#customers .customerCard').length;
  			console.log(n);
  			if ($('#customers .customerCard .pull-right').html()=="#5" && $('#customers .customerCard').length == 1){
  				console.log('это 5-й'); 
  				var d = $('#search-customer-form-group').css("display");
				console.log(d);
				
					  				//$('#customers .customerCard .pull-right').html('# 5');
				$('#customer_part button.setup-customer').removeClass('setup-customer').addClass('change-customer').html('<i class="icon-refresh"></i>&nbsp;{l s="Change"}').blur();
				$('#customer_part button.setup-customer').closest('.customerCard').addClass('selected-customer');
				...
				$('#old_carts_orders').css("height", "auto");
				$('#old_carts_orders').removeClass("in");

				
				//
				$('#products_part input#product').focus();
		...
		$('#products_found #product_list').bind("DOMSubtreeModified",function(){
			console.log('найдены товары');
			if ($('#products_found #product_list option').size()==1)
			{
				console.log('найден ровно один товар');
				addProduct();
			}
			if ($('#products_found #product_list option').size()>1) {
				alert ('найдено несколько товаров ('+$('#products_found #product_list option').size()+')!');
			}
		...
		$('#product').on("keyup keypress", function(e) {
	...
	function updateQty(id_product, id_product_attribute, id_customization, qty)
	{
		$.ajax({
			...
			success : function(res)
			{
				...
				{
					$('#products_err').addClass('hide');
					$( "#added-tocart-message" ).show('slow').delay(1000).fadeOut('slow');
					$("#products_part input#product[type='text']").select();
				}
			...
			<div class="col-lg-9">
				<div class="row">
					...
					<div class="col-lg-6">
						...
						<button class=" btn btn-default setup-retail-customer" id="setup-retail-customer">
							<i class="icon-plus-sign-alt"></i>
							{l s='Розничный покупатель'}
						</button>
					</div>
				</div>
			</div>
						...
						<select name="payment_module_name" id="payment_module_name">
							{foreach from=$payment_modules item='module'}
								<option value="{$module->name}" 
									{*
									{if isset($smarty.post.payment_module_name) && $module->name == $smarty.post.payment_module_name}selected="selected"{/if}
									*}
									{if $module->name == "cashondelivery"}selected="selected"{/if}
									>{$module->displayName}</option>
							{/foreach}
						</select>
					</div>
				</div>
