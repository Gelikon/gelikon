		...
		if ($this->display == 'addstock' || $this->display == 'removestock' || $this->display == 'transferstock')
		{
			if (Tools::isSubmit('id_product') || Tools::isSubmit('id_product_attribute'))
			{
				...
				if ($product_is_valid === true && $is_pack == false && $is_virtual == false)
				{
                    ...
                    if(!$id_warehouse)$id_warehouse=1;
					...
					if ($this->display == 'transferstock')
					{
                        ...
                        if(!$id_warehouse_from)$id_warehouse_from = 1;

                        $id_warehouse_to = Tools::getValue('id_warehouse_to', '');
                        if(!$id_warehouse_to)$id_warehouse_to = 2;

						$helper->fields_value['id_warehouse_from'] = $id_warehouse_from;
						$helper->fields_value['id_warehouse_to'] = $id_warehouse_to;
						$helper->fields_value['usable_from'] = Tools::getValue('usable_from', '1');
						$helper->fields_value['usable_to'] = Tools::getValue('usable_to', '1');
                        $helper->fields_value['quantity'] = Tools::getValue('quantity', '1');
					}
