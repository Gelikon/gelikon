...
class ProductController extends ProductControllerCore
{
		...
		else
		{
			...
			{       
                                if($this->context->language->id != 1)
                                    $this->product->name = $this->rus2translit($this->product->name);
				// Load category
				$id_category = false;
	...
	public function initContent()
	{
			...
			*/ 
			...
			if ($all_warehouses) {
				...
				foreach($all_warehouses as $warehouse) {
					...
					$real_quantity = $manager->getProductRealQuantities(
							(int)$this->product->id,
							0, //$item['id_product_attribute']
							$id_warehouse,
							true
						);
					if ($real_quantity>0)
		...
		$context = Context::getContext();
            if(isset($context->cookie->shut_off_transit) && $context->cookie->shut_off_transit == 1){
               return $string;
            }
    $converter = array(
        'а' => 'a',   'б' => 'b',   'в' => 'v',
